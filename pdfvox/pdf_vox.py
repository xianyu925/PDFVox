...
class PDFVox:
    ...
    def _stream_chat_response(
        ...
        if isinstance(user_prompt, list) and all(
            ...
            content_items.append({"role": "user", "content": user_prompt})
        else:
            # Text-only format
            content_items.append({"role": "user", "content": user_prompt})
        ...
        explanation = ""
        with requests.post(
            url, headers=headers, json=data, stream=True, timeout=180
        ) as response:
    ...
    def understand(self, content, course_name="机器学习导论"):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    user_prompt = [
                        {
                            "type": "text",
                            "text": f"请为以下{course_name}课程PPT的第{page}页内容生成一段课堂讲解，要求：\n"
                            "1. 以大学教授的语气和风格讲解\n"
        ...
        for idx, item in enumerate(explanations):
            ...
            next_exp = (
                explanations[idx + 1]["explanation"] if idx < total_pages - 1 else ""
            )
    ...
    def save_explanations_to_txt(
        self, explanations, output_dir, file_name="explanations.txt"
    ):
                ...
                try:
                    ...
                    with requests.post(
                        ...
                        if "application/json" in content_type.lower():
                            ...
                            if (
                                isinstance(audio_base64, dict)
                                and "base64" in audio_base64
                            ):
                                audio_base64 = audio_base64.get("base64")

                            if isinstance(
                                audio_base64, str
                            ) and audio_base64.startswith("data:"):
                            ...
                            print(
                                f"[Page {page}] 已写入 {bytes_received} 字节（JSON格式音频）",
                                flush=True,
                            )
                        else:
                            with open(audio_path, "wb") as f:
                                ...
                                for chunk in response.iter_content(chunk_size=4096):
                                    if chunk:
                                        ...
                                        if bytes_received % (1024 * 50) < 4096:
                                            print(
                                                f"[Page {page}] 已接收 {bytes_received} 字节",
                                                flush=True,
                                            )
    ...
    def process(self, pdf_path, output_dir, course_name="机器学习导论"):
        """Process PDF file completely"""
        print("Starting PDF processing...")

        content = self.analysis(pdf_path)
        explanations = self.understand(content, course_name=course_name)
